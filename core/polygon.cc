...
#define __DEBUG_TIKZ__
    ...
    while (std::count (clipped.cbegin(), clipped.cend(), false) > 3) {
        ...
        if (numeric::close_enough (q_diff, M_PI) || numeric::close_enough (q_diff, -M_PI)) {
        ...
        // 2. Is the line segment (vp, vn) free from intersection with other line segments in
        // the polygon?
        //
...
// Assume that the points form a closed polygon, i.e., the first and last elements
// coincide.
...
Polygon::determine_interior_direction (const Points& points) const {
    ...
    if (numeric::close_enough (sum_rotational_angles, 2 * M_PI)) return InteriorDirection::left;
    if (numeric::close_enough (sum_rotational_angles, -2 * M_PI)) return InteriorDirection::right;
...
//
// NOTE: when this function returns (0, 0, 0), it means it failed to find the vertices.
