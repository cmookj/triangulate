...
class Polygon {
 ...
 public:
  ...
  std::string interior_direction() const;

 private:
  void increase_idx(std::size_t& idx) const;

  // Distinguish interior and outside of polygon
  // Assume that the points form a closed polygon, i.e., the first and last
  // elements coincide.
  InteriorDirection determine_interior_direction(const Points& points) const;

  void register_triangle(Triangles& triangles, const std::size_t a,
                         const std::size_t b, const std::size_t c) const;

  // Find the index to a vertex which is not clipped, from offset.
  std::optional<std::size_t> index_unclipped_vertex(
      const std::vector<bool>& clipped, const std::size_t offset = 0) const;
  ...
  // vertices.
  ThreeIndices three_unclipped_vertices(const std::vector<bool>& clipped,
                                        const std::size_t offset = 0) const;

  bool has_intersection(const std::vector<bool>& clipped,
                        const std::size_t idx_vp, const std::size_t idx_v,
                        const std::size_t idx_vn) const;

  void open_debug_tikz(const std::string& filename) const;

  void close_debug_tikz() const;

  void append_debug_tikz(const std::vector<bool>& flags,
